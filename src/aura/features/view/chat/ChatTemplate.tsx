...
interface ChatTemplateProps {
    onSendTemplate: (phoneNumber: string, ddi: string, templateId: string, params: string[]) => void;
    onClose: () => void;
}
...
const ChatTemplate: React.FC<ChatTemplateProps> = ({ onSendTemplate, onClose }) => {
    ...
    useEffect(() => {
        if (selectedTemplate) {
            setTemplateParams(new Array(selectedTemplate.paramCount).fill(''));
        } else {
            setTemplateParams([]);
        }
    }, [selectedTemplate]);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (selectedTemplate.paramCount > 0 && templateParams.some(p => !p || p.trim() === '')) {
            alert('Por favor, preencha todos os parâmetros do template.');
            return;
        }

        setPhoneError(null);
        console.log('Enviando template:', { ddi, phoneNumber, selectedTemplateId, templateParams });

        onSendTemplate(phoneNumber, ddi, selectedTemplateId, templateParams.slice(0, selectedTemplate.paramCount));

        setPhoneNumber('');
        setSelectedTemplateId(null);
        setTemplateParams([]);
        // onClose(); // Descomente se quiser fechar automaticamente após o envio
    };
    ...
    return (
        <div className="chat-template-panel">
            <div className="chat-template-header">
                ...
                <button onClick={onClose} className="chat-template-close-button" title="Fechar">
            ...
            <form onSubmit={handleSubmit} className="chat-template-form">
                <div className="chat-template-alert">
                    <span className="chat-alert-icon">⚠️</span>
                    <span>Certifique-se de que o número está correto</span>
                </div>

                <div className="chat-template-field phone-field">
                    <label htmlFor="phoneDdiCombined">Número de Telefone</label>
                    ...
                    {phoneError && <p className="chat-template-error-message">{phoneError}</p>}
                    {!phoneError && !phoneNumber.trim() && (
                        <p className="chat-template-info-message">Digite número de telefone do contato.</p>
                    )}
                </div>

                <div className="chat-template-field">
                    <label className="template-section-label">Escolha a mensagem desejada.</label>
                        ...
                        {availableTemplates.map((template) => (
                            <div key={template.id} className="template-option">
                                ...
                                <label htmlFor={template.id} className="template-radio-label">
                                    {template.text.split(/\{\{\d+\}\}/g).map((part, index, arr) => (
                                        <React.Fragment key={index}>
                                            {part}
                                            {index < template.paramCount && index < arr.length -1 && (
                                                <span className="template-param-placeholder">{`{{${index + 1}}}`}</span>
                                            )}
                                        </React.Fragment>
                                    ))}
                                </label>
                ...
                {selectedTemplate && selectedTemplate.paramCount > 0 && (
                    <div className="chat-template-field template-params-section">
                        <label className="template-section-label">Preencha os parâmetros:</label>
                        {Array.from({ length: selectedTemplate.paramCount }).map((_, index) => (
                            <div key={index} className="template-param-input-field">
                                <label htmlFor={`param-${index + 1}`}>{`Parâmetro {{${index + 1}}}:`}</label>
                                <input
                                    type="text"
                                    id={`param-${index + 1}`}
